//
// Created by XianY on 2023/3/16.
//

#ifndef ULTRA_VISION_FRAMEWORK_CRC_16_HPP
#define ULTRA_VISION_FRAMEWORK_CRC_16_HPP

#include "array"
#include "utils/mcu/serial/communication.h"

class Crc16 {
   public:
    template <std::size_t LEN>
    static std::array<Byte, 2> Encode(const std::array<Byte, LEN>& buffer) {
        Byte crc_high{0xFF};
        Byte crc_low{0xFF};
        for (auto i = 0UL; i < LEN; i++) {
            Byte index = crc_high ^ buffer[i];
            crc_high = crc_low ^ CrcTableHigh[index];
            crc_low = CrcTableLow[index];
        }

        return {crc_low, crc_high};
    }

    template <std::size_t LEN>
    static bool Verify(const std::array<Byte, LEN>& buffer, const std::array<Byte, 2>& to_check) {
        auto buffer_check = Crc16::Encode(buffer);
        return (buffer_check == to_check);
    }

   private:
    static constexpr Byte CrcTableHigh[]{
        

    };

    static constexpr Byte CrcTableLow[]{
        };
};

#endif  // ULTRA_VISION_FRAMEWORK_CRC_16_HPP
